...
class Cube:
    ...
    def swap_x(self, t1, t2, t3, t4):
        ...
        backup = np.array(["", "", ""])
        self.copy_stickers(backup, self.faces[t4[0]][t4[1]])
        self.copy_stickers(self.faces[t4[0]][t4[1]], self.faces[t3[0]][t3[1]])
        self.copy_stickers(self.faces[t3[0]][t3[1]], self.faces[t2[0]][t2[1]])
        self.copy_stickers(self.faces[t2[0]][t2[1]], self.faces[t1[0]][t1[1]])
        self.copy_stickers(self.faces[t1[0]][t1[1]], backup)

    def copy_stickers(self, destination, origin):
    ...
    def swap_y(self, t1, t2, t3, t4):
        ...
        # self.faces[t4[0]][:, t4[1]]  - example: self.faces[FRONT][:, 0] - get values of FRONT faces at column 0
        if t4[2]:
            self.copy_stickers(backup, np.flip(self.faces[t4[0]][:, t4[1]]))
        else:
            self.copy_stickers(backup, self.faces[t4[0]][:, t4[1]])
        if t3[2]:
            self.copy_stickers(self.faces[t4[0]][:, t4[1]], np.flip(self.faces[t3[0]][:, t3[1]]))
        else:
            self.copy_stickers(self.faces[t4[0]][:, t4[1]], self.faces[t3[0]][:, t3[1]])
        if t2[2]:
            self.copy_stickers(self.faces[t3[0]][:, t3[1]], np.flip(self.faces[t2[0]][:, t2[1]]))
        else:
            self.copy_stickers(self.faces[t3[0]][:, t3[1]], self.faces[t2[0]][:, t2[1]])
        if t1[2]:
            self.copy_stickers(self.faces[t2[0]][:, t2[1]], np.flip(self.faces[t1[0]][:, t1[1]]))
        else:
            self.copy_stickers(self.faces[t2[0]][:, t2[1]], self.faces[t1[0]][:, t1[1]])
        self.copy_stickers(self.faces[t1[0]][:, t1[1]], backup)
    ...
    #         state.append(f"{k}:  {v[0, 0]} {v[0, 1]} {v[0, 2]}")
    #         state.append(f" {v[1, 0]} {v[1, 1]} {v[1, 2]}"),
    #         state.append(f"  {v[2, 0]} {v[2, 1]} {v[2, 2]}")
